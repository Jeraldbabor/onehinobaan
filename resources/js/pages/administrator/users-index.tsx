...
import { Plus, Search, MoreHorizontal, UserCog, Key, Trash2, ShieldCheck, UserCheck, User as UserIcon, Camera } from 'lucide-react';
...
export default function UsersIndex({ users }: UsersIndexProps) {
    ...
    const getRoleBadge = (role: string) => {
        switch (role) {
            case 'admin':
                return <span className="inline-flex items-center rounded-full bg-red-100 px-2 py-0.5 text-xs font-medium text-red-800 dark:bg-red-900/30 dark:text-red-400">Admin</span>;
            case 'editor':
                return <span className="inline-flex items-center rounded-full bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">Editor</span>;
            default:
                return <span className="inline-flex items-center rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-800 dark:bg-slate-800 dark:text-slate-400">User</span>;
        }
    };
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-1 flex-col gap-6 px-4 py-8 md:px-6 lg:max-w-5xl">
                <header className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight">Accounts & Permissions</h1>
                        <p className="mt-1 text-sm text-muted-foreground">
                            Manage administrative and editor accounts for the municipality.
                        </p>
                    </div>
                    ...
                    {users.length === 0 ? (
                        <Card>
                            <CardContent className="flex flex-col items-center justify-center py-12">
                                ...
                                <p className="mt-4 text-sm text-muted-foreground">No other accounts found.</p>
                            </CardContent>
                        </Card>
                    ) : (
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 lg:p-6">
                                        <div className="flex items-start gap-4">
                                            ...
                                            <div className="space-y-1">
                                                ...
                                                <p className="text-xs text-muted-foreground/60">Joined {user.created_at}</p>
                                            </div>
                                        </div>
            ...
            <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        ...
                        <DialogDescription>Add a new administrator, editor, or regular user.</DialogDescription>
                    </DialogHeader>
                    <form onSubmit={submitAdd} className="space-y-4 pt-4">
                        <div className="flex flex-col items-center gap-4 mb-4">
                            <div className="relative">
                                <div className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-full bg-muted shadow-inner">
                                    {addAvatarPreview ? (
                                        <img src={addAvatarPreview} alt="Preview" className="h-full w-full object-cover" />
                                    ) : (
                            ...
                            <div className="text-center">
                                ...
                                {addForm.errors.avatar && <p className="text-[10px] text-destructive mt-1">{addForm.errors.avatar}</p>}
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <Select
                                ...
                                onValueChange={(val: any) => addForm.setData('role', val)}
                            >
                        ...
                        <DialogFooter className="pt-4">
                            ...
                            <Button type="submit" disabled={addForm.processing}>Create Account</Button>
                        </DialogFooter>
                    </form>
                </DialogContent>
            </Dialog>
                    ...
                    <form onSubmit={submitEdit} className="space-y-4 pt-4">
                        <div className="flex flex-col items-center gap-4 mb-4">
                            ...
                            <div className="text-center">
                                ...
                                {editForm.errors.avatar && <p className="text-[10px] text-destructive mt-1">{editForm.errors.avatar}</p>}
                            </div>
                        </div>
                ...
                <DialogContent className="sm:max-w-[425px]">
                    <form onSubmit={handleResetPassword}>
                        ...
                        <div className="grid gap-4 py-4">
                            <div className="grid gap-2">
                                ...
                                <Input
                                    ...
                                    required
                                />
                                {resetForm.errors.password && <p className="text-xs text-destructive">{resetForm.errors.password}</p>}
                            </div>
                            <div className="grid gap-2">
                        ...
                        <DialogFooter>
                            ...
                            <Button type="submit" variant="destructive" disabled={resetForm.processing}>Reset Password</Button>
                        </DialogFooter>
                    </form>
                </DialogContent>
            </Dialog>

            <Toast open={showToast} onOpenChange={setShowToast} title={toastMessage} />
        </AppLayout>
    );
}
